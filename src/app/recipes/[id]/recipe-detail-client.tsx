    ...
    <div className="min-h-dvh">
      ...
      <div className="px-4 pt-4">
        ...
        <div className="mt-4 flex gap-3">
          <div className="flex-1 rounded-xl border border-[#ECECEC] p-3 text-center">
            <p className="text-lg font-bold">{recipe.time_minutes}</p>
            <p className="text-xs text-[#6B6B6B]">min</p>
          </div>
          <div className="flex-1 rounded-xl border border-[#ECECEC] p-3 text-center">
            <p className="text-lg font-bold">{recipe.kcal}</p>
            <p className="text-xs text-[#6B6B6B]">kcal</p>
          </div>
          <button
            onClick={() => {
            ...
            className="flex-1 rounded-xl border border-[#ECECEC] p-3 text-center"
          >
            <p className="text-lg font-bold">{servings}</p>
            <p className="text-xs text-[#6B6B6B]">serves</p>
          </button>
        </div>

        {/* Tabs */}
        <div className="mt-6 flex border-b border-[#ECECEC]">
          {(["ingredients", "steps", "notes"] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 pb-2 text-sm font-medium capitalize ${
                activeTab === tab ? "border-b-2 border-[#1A1A1A] text-[#1A1A1A]" : "text-[#A0A0A0]"
              }`}
            >
              {tab}
            </button>
        ...
        <div className="mt-4 pb-24">
            ...
            <div>
              {recipe.ingredients.map((ing, i) => (
                <button
                  ...
                  className="flex w-full items-center gap-3 border-b border-[#ECECEC] py-3"
                >
                  <div className={`h-5 w-5 rounded border-2 ${checkedIngredients.has(i) ? "border-[#1A1A1A] bg-[#1A1A1A]" : "border-[#D0D0D0]"}`}>
                    {checkedIngredients.has(i) && <span className="text-xs text-white">✓</span>}
                  </div>
                  <span className={`flex-1 text-left text-sm ${checkedIngredients.has(i) ? "text-[#A0A0A0] line-through" : ""}`}>
                    {ing.name} {ing.optional && <span className="text-[#A0A0A0]">(optional)</span>}
                  </span>
              ...
              <div className="mt-4 flex flex-col gap-3">
                <button
                  ...
                  className="rounded-lg border-2 border-dashed border-[#D0D0D0] py-3 text-sm font-medium text-[#6B6B6B]"
                >
                  Add missing to list
                </button>
                <Link
                  href={`/chat?prompt=What can I use instead of an ingredient in ${recipe.title}?`}
                  className="rounded-lg border-2 border-dashed border-[#D0D0D0] py-3 text-center text-sm font-medium text-[#6B6B6B]"
                >
                  Swap an item
                </Link>
              </div>
            ...
            <div className="flex flex-col gap-4">
              {recipe.steps.map((step, i) => (
                <div key={i} className="rounded-xl border border-[#ECECEC] p-4">
                  <p className="text-xs font-medium text-[#A0A0A0]">Step {i + 1}</p>
                  <h3 className="mt-1 font-semibold">{step.title}</h3>
                  <p className="mt-2 text-sm text-[#6B6B6B]">{step.body}</p>
                  {step.timer_seconds && (
          ...
          {activeTab === "notes" && (
            <p className="text-sm leading-relaxed text-[#6B6B6B]">{recipe.notes}</p>
          )}
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-1/2 z-50 w-full max-w-[480px] -translate-x-1/2 border-t border-[#ECECEC] bg-white p-4">
        <div className="flex gap-3">
          ...
          <Link
            href={`/cook/${recipe.id}`}
            className="flex flex-1 items-center justify-center rounded-lg bg-[#1A1A1A] py-3 text-sm font-semibold text-white"
          >
            Start cooking
          </Link>
        </div>
