...
export default function CookDonePage() {
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-white px-6 text-center">
      ...
      <h1 className="font-[family-name:var(--font-playfair)] mt-4 text-2xl font-bold text-[#111111]">Nice work!</h1>
      <p className="mt-2 text-sm text-[#6B7280]">How was it? Rate this cook.</p>

      <div className="mt-6 flex gap-2">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} onClick={() => setRating(n)} aria-label={`Rate ${n} star${n > 1 ? "s" : ""}`}>
            <Star
              className={`h-10 w-10 ${n <= rating ? "fill-yellow-400 text-yellow-400" : "text-[#E5E7EB]"}`}
            />
          </button>
        ))}
      </div>
      ...
      <button
        ...
        className="mt-8 w-full rounded-lg bg-[#2563EB] py-3.5 text-sm font-semibold text-white disabled:opacity-30"
      >
