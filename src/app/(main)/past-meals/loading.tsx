  ...
  return (
    <div className="px-4 pt-12 pb-4 animate-pulse">
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-16 rounded-xl border border-dashed border-[#D0D0D0]" />
        ))}
      </div>
      <div className="mt-6 space-y-3">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex items-center gap-3 rounded-xl border border-[#ECECEC] p-3">
            <div className="h-16 w-16 rounded-lg bg-[#ECECEC]" />
