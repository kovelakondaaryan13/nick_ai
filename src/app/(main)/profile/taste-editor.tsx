      ...
      <div className="flex flex-wrap gap-2">
        {flavors.map((f) => (
          <button
            key={f}
            onClick={() => remove(f)}
            className="flex items-center gap-1 rounded-full bg-[#2563EB] px-3 py-1.5 text-xs font-medium capitalize text-white"
          >
            {f}
            <X className="h-3 w-3" />
          </button>
        ...
        {available.length > 0 && (
          <button
            ...
            className="flex items-center gap-1 rounded-full border border-dashed border-[#E5E7EB] px-3 py-1.5 text-xs font-medium text-[#6B7280]"
          >
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {available.map((f) => (
            <button
              key={f}
              onClick={() => add(f)}
              className="rounded-full border border-[#E5E7EB] px-3 py-1.5 text-xs font-medium capitalize text-[#111111]"
            >
              {f}
            </button>
          ))}
